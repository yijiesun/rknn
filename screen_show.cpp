...
int main(int argc, char *argv[])
{
   ...
   sigaction(SIGINT, &sigIntHandler, NULL);
    ...
    while(1){
        ...
        Mat img;
        cv::resize(frame, img, cv::Size(img_width, img_height), (0, 0), (0, 0), cv::INTER_LINEAR);
        ...
        if(outputs[0].size == outputs_attr[0].n_elems*sizeof(float) && outputs[1].size == outputs_attr[1].n_elems*sizeof(float))
        {
            ...
            if (validCount < 100) {
                ...
                for (int i = 0; i < validCount; ++i) {
                    ...
                    int x1 = static_cast<int>(predictions[n * 4 + 1] * show_img.cols);
                    int y1 = static_cast<int>(predictions[n * 4 + 0] * show_img.rows);
                    int x2 = static_cast<int>(predictions[n * 4 + 3] * show_img.cols);
                    int y2 = static_cast<int>(predictions[n * 4 + 2] * show_img.rows);

                    string label = labels[topClassScoreIndex];

                    std::cout << label << "\t@ (" << x1 << ", " << y1 << ") (" << x2 << ", " << y2 << ")" << "\n";

                    rectangle(show_img, Point(x1, y1), Point(x2, y2), colorArray[topClassScoreIndex%10], 3);
                    putText(show_img, label, Point(x1, y1 - 12), 1, 2, Scalar(0, 255, 0, 255));
                }
